...
class ScanResultScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final registrantAsync = ref.watch(
      registrantDetailsProvider(
          (departmentCode: departmentCode, registrantId: qrValue)),
    );
  ...
  Future<void> _markAttendance(
      BuildContext context, WidgetRef ref, String matric, bool present) async {
    try {
      final api = ref.read(departmentsApiProvider);
      final dateStr = DateFormat('yyyy-MM-dd').format(DateTime.now());
      await api.markAttendanceById(departmentCode, matric, present,
          date: dateStr);
      NotificationService.showSuccess(
          present ? 'Attendance marked present' : 'Attendance denied');
      if (context.mounted) {
        context.pop();
      }
    } catch (e) {
